...
const App = memo(function App({ data }) {
  ...
  const getIndexData = async (country, code, func) => {
    const _indexData = await axios.get(
    ...
    if (
      ...
      _dailyData.data.data &&
      _indexData.data.data.length > 0 &&
      _dailyData.data.data.length > 0
    ) {
      let _result = getNewData(
        _indexData.data.data[0],
        _dailyData.data.data[0]
      );
  ...
  function getNewData(_today, _yesterday) {
    return {
      ...
      diffPercent: getNumber(
        ((_today.close - _yesterday.close) / _yesterday.close) * 100
      ),
      open: _today.open,
      highest: _today.high,
      ...
      amplitude: getNumber(
        ((_today.high - _today.low) / _yesterday.close) * 100
      ),
      changes: getNumber(_today.close - _yesterday.close),
      changePercent: getNumber(
        ((_today.close - _yesterday.close) / _yesterday.close) * 100
      ),
    };
  }
    ...
    <>
      <div className={styles.card1}>
        ...
        <div
          ...
          style={{
            backgroundColor:
              data1.diff > 0 ? "rgb(230, 79, 39)" : "rgb(88, 108, 246)",
          }}
        ></div>
        <div className={styles.value}>{data1.value}</div>
        <div className={styles.diff}>
          {data1.diff > 0 ? "+" + data1.diff : data1.diff}
        </div>
        <div className={styles.diffPercent}>
          {data1.diffPercent > 0 ? "+" + data1.diffPercent : data1.diffPercent}%
        </div>
      ...
      <div className={styles.card2}>
        ...
        <div
          ...
          style={{
            backgroundColor:
              data2.diff > 0 ? "rgb(230, 79, 39)" : "rgb(88, 108, 246)",
          }}
        ></div>
        <div className={styles.value}>{data2.value}</div>
        <div className={styles.diff}>
          {data2.diff > 0 ? "+" + data2.diff : data2.diff}
        </div>
        <div className={styles.diffPercent}>
          {data2.diffPercent > 0 ? "+" + data2.diffPercent : data2.diffPercent}%
        </div>
      ...
      <div className={styles.card3}>
        ...
        <div
          ...
          style={{
            backgroundColor:
              data3.diff > 0 ? "rgb(230, 79, 39)" : "rgb(88, 108, 246)",
          }}
        ></div>
        <div className={styles.value}>{data3.value}</div>
        <div className={styles.diff}>
          {data3.diff > 0 ? "+" + data3.diff : data3.diff}
        </div>
        <div className={styles.diffPercent}>
          {data3.diffPercent > 0 ? "+" + data3.diffPercent : data3.diffPercent}%
        </div>
